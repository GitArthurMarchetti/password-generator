...
// CORREÇÃO 1: Divisão ajustada para nunca retornar 1.0 (evita caracteres inválidos)
function secureMathRandom() {
    return window.crypto.getRandomValues(new Uint32Array(1))[0] / (Math.pow(2, 32));
}

// Generator Functions
// CORREÇÃO 2: Todas as funções agora usam secureMathRandom() para consistência
...
function generatePassword(length, lower, upper, number, symbol) {
    ...
    // CORREÇÃO 3: O loop agora incrementa pelo número de tipos (i += typesCount)
    // Isso impede que gere caracteres demais.
    ...
    // Melhoria no Shuffle (opcional, mas recomendada para evitar padrões simples)
